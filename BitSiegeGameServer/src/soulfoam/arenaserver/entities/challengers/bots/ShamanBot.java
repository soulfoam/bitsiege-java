...
public class ShamanBot extends Bot{

	public Entity targetAllyPlayer;
	
	public ShamanBot(float x, float y) {
		
		this.challengerType = Bot.SHAMANCLASSBOT;
		this.x = x;
		this.y = y;
		
		this.bot = true;
		this.skinID = CosmeticLibrary.SHAMAN_SKIN_NORMAL;
		this.underglowID = -1;
		initAbilities();
		this.width = 32;
		this.height = 24;
		...
		if (!dead){
			...
			if (fighting){
				...
				if (targetAllyPlayer == null){
					...
					castAbility(2, targetAllyPlayer.getX(), targetAllyPlayer.getY());
				}
				if (targetPlayer != null){
					castAbility(3, targetPlayer.getX(), targetPlayer.getY());
					castAbility(4, targetPlayer.getX() + Res.randInt(-4 * Tile.TILE_SIZE, 4 * Tile.TILE_SIZE), targetPlayer.getY() + Res.randInt(-4 * Tile.TILE_SIZE, 4 * Tile.TILE_SIZE));
				}
			}
			else{
