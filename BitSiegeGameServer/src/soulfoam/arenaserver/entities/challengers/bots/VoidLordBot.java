...
public class VoidLordBot extends Bot{
	...
	public VoidLordBot(float x, float y) {
		
		this.challengerType = Bot.VOIDLORDCLASSBOT;
		this.x = x;
		this.y = y;
		
		this.bot = true;
		this.skinID = CosmeticLibrary.VOIDLORD_SKIN_NORMAL;
		this.underglowID = -1;
		initAbilities();
		this.width = 32;
		this.height = 24;
		...
		if (!dead){
			...
			if (fighting){
				...
				if (targetPlayer != null){
					castAbility(2);
					if (targetPlayer != null && getBounds().intersects(targetPlayer.getBounds())) {
						castAbility(3);
						castUlt();
						castAbility(4);
					}
